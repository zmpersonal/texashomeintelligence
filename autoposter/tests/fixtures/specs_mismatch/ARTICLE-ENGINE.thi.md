...
the skeleton defines the process; `PUBLISH-TARGET.thi.md` defines where it publishes. All three are
required — the engine HALTS if this file or the target is missing.

- **Site:** texashomeintelligence.com  · **Publish target:** `PUBLISH-TARGET.thi.md` (self-id:
...
## Handoff
Publish via `PUBLISH-TARGET.thi.md` (deploy-on-command, 🔴). On live URL, hand to `social-autoposter`
