 ...
 * Travis County, TX (state 48, county 453). ACS 5-year is an annual
 * release, not a fast-changing feed — one observation per vintage year,
 * keyed by that year, updated in place if re-run within the same year.
 *
...
export const censusAcs: FetcherModule<HousingStockValue> = {
  ...
  async fetchRaw(_ctx): Promise<Observation<HousingStockValue>[]> {
    ...
    url.searchParams.set("for", `county:${COUNTY_FIPS}`);
    ...
    if (!data) {
      throw new Error(`Census ACS returned no data row for state=${STATE_FIPS} county=${COUNTY_FIPS}`);
    }
