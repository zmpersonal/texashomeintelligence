...
// --- data sources: priority feeds must be "sample", not "stub" or "live" ---
...
for (const id of expectedPriority) {
  const entry = dataSources.find((d) => d.id === id);
  if (!entry) fail(`data-sources: missing priority feed "${id}"`);
  else if (entry.status !== "sample") {
    fail(`data-sources/${id}: priority feed should be status "sample", got "${entry.status}"`);
  }
}
for (const d of dataSources) {
  if (d.status === "live") {
    fail(`data-sources/${d.id}: status is "live" — nothing should be live yet in Phase 1`);
  }
}
